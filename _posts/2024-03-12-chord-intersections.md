...
For Figure 3, we would do the following: open 0, open 1, open 2, close 0 (add two intersections since chords 1 and 2 are open), open 3, close 2 (add one intersection since chord 3 is open), close 3, close 1. This gives $I=2+1=3$, which agrees with the diagram. 

When our loop thru $P$ brings us to an endpoint of chord $i$, we can use a hashset `h` to determine whether to open or close $i$. If $i$ isn't a key in `h`, we can set `h[i]=open`, otherwise set `h[i]=closed`. When we have to close $i$, the hard part is determining how many higher-numbered chords are still open. 
